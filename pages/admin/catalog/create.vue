<template>
  <div id="catalog-create">
    ...
    <div class="catalog-create_form">
      <el-form label-position="right" label-width="200px" :model="createForm" :rules="rules" ref="createForm">
        ...
        <el-form-item label="Цена">
          <el-input type="number" v-model.number="createForm.price"></el-input>
        </el-form-item>
        <el-form-item label="Выгода">
          <el-input type="number" v-model="createForm.benefit"></el-input>
        </el-form-item>
        <el-form-item label="Платеж в месяц">
          <el-input type="number" v-model="createForm.credit"></el-input>
        </el-form-item>
        <el-form-item label="Мощность">
          <el-input type="number" v-model="createForm.power"></el-input>
        </el-form-item>
        <el-form-item label="Разгон до 100 км/ч">
          <el-input type="number" v-model="createForm.speed"></el-input>
        </el-form-item>
        <el-form-item label="Расход топлива">
          <el-input type="number" v-model="createForm.gasoline"></el-input>
        </el-form-item>
        <el-form-item label="SALE" prop="sale">
          <el-switch v-model="createForm.sale"> </el-switch>
        </el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        valid ? this.addCar() : false;
      });
    },
    addCar() {
      this.$axios
        .post(`/api/cars/`, this.createForm)
        .then((res) => {
          this.$message({
    ...
    handleRemove(file, fileList) {
      ...
      this.createForm.photos = fileList1;
    },
    ...
    removeColorImg(file, fileList) {
      const fileList1 = fileList;
      fileList.forEach((item, index) => (fileList1[index].url = String(item.response).split("static")[1]));
      this.createForm.colors = fileList1;
    },
    ...
    updateImageCar(response, file, fileList) {
      this.createForm.image = window.location.origin + response.split("static")[1];
    },
  },
};
</script>
